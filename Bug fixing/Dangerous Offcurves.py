			...
			for node in path.nodes:
				...
				if node.type == 'offcurve' and node.prevNode.type != 'offcurve':
					...
					if hIntersection1 and vIntersection1 and hIntersection2 and vIntersection2:
						...
						if (
							abs(hIntersection1.x - node.x) < THRESHOLD
							or abs(vIntersection1.y - node.y) < THRESHOLD
							or abs(hIntersection2.x - node.nextNode.x) < THRESHOLD
							or abs(vIntersection2.y - node.nextNode.y) < THRESHOLD
						):
							# collect layers
